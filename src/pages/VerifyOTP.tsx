...
export default function VerifyOTP() {
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      const { error: verifyError } = await supabase.auth.verifyOtp({ type: "sms", token: otp.trim(), phone });
      if (verifyError) { setError(/expired|invalid/i.test(verifyError.message) ? "OTP expired or invalid." : verifyError.message); return; }
  ...
  const handleResendOTP = async () => {
    ...
    } catch (e: any) { setError(e.message); } finally { setResendLoading(false); }
  };
      ...
      <div className="w-full max-w-md space-y-4">
        ...
        <Card className="shadow-card border-pink-100 gradient-card backdrop-blur-sm">
          <CardContent className="p-6">
            <Button variant="ghost" size="sm" onClick={() => navigate('/auth')} className="mb-4 p-2"><ArrowLeft className="w-5 h-5" /></Button>
