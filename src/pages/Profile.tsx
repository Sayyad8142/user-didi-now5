                  ...
                  {!isEditing ? (
                    hydrating || (communitiesLoading && !profile?.community) ? (
                      ...
                      <p className="text-lg font-semibold text-gray-900">
                        {isValidDisplayName(profile?.full_name) ? "'''Do not make any visual modifications. The phrases I write are commands to understand what I want, not to be written down. Understand their content well, then execute what is required.'''\n                                        \n                                            \n                                            User App – Fix Profile Community Name\n\nIssue\n\nOn the User App → Profile screen, the Community field is displaying random/incorrect text instead of the user's actual assigned community name.\n\nFor example, it is currently showing a long unrelated text string, while it should show the real community name such as Prestige High Fields.\n\nRequired Fix\n\nDo not hardcode or manually replace the displayed text.\n\nTrace where the Profile screen is getting the community value from.\n\nFetch the user's actual community using the authoritative backend relationship, preferably:\nuser/profile → community_id → communities table → community name\n\nDisplay only the community's actual name in the Community field.\n\nDo not display raw IDs, metadata, prompts, descriptions, notes, or any other database field.\n\nMake sure the Profile screen refreshes correctly if the user's assigned community changes.\n\nIf no community is assigned, display \"Not assigned\" instead of random/fallback data.\n\nDo not make unnecessary visual/UI changes to the Profile screen.\n\nImportant Backend Audit\n\nPlease investigate how this incorrect text entered the Community field in the first place.\n\nCheck:\n\nWhich database column currently contains this incorrect value.\n\nWhether the user profile is mapped to the wrong field.\n\nWhether community creation/update logic accidentally stored unrelated text as the community name.\n\nWhether other users are affected by the same issue.\n\nIf corrupted data exists, fix the affected records safely after identifying the root cause, and prevent the same corruption from happening again.\n\nExpected Result\n\nCOMMUNITY\nPrestige High Fields\n\nThe value must always come from the authoritative community record associated with that user, never from arbitrary profile text or client-side fallback values." : (communities.find(c => c.id === profile?.community_id || c.value === profile?.community)?.name
                          || profile?.community
                          || 'Not provided')}
                      </p>
                    )
        ...
        <div className="space-y-3">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="font-medium text-gray-700">FAQs & Help Center</span>
            </div>
            <div className="text-gray-400">›</div>
          </Link>
