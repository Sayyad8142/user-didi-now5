...
-- Update the push notification trigger to include completion_otp in "Worker Assigned" push body
...
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' AND OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    IF NEW.status IN ('assigned', 'accepted', 'confirmed') AND OLD.status NOT IN ('assigned', 'accepted', 'confirmed') THEN
      ...
      IF NEW.completion_otp IS NOT NULL THEN
        v_body := COALESCE(NEW.worker_name, 'A worker') || ' assigned for your ' || NEW.service_type || ' | OTP: ' || NEW.completion_otp;
      ELSE
      ...
      PERFORM net.http_post(
        ...
        body := jsonb_build_object(
          ...
          'data', jsonb_build_object(
            ...
            'completion_otp', COALESCE(NEW.completion_otp, '')
          )
        )
      );
