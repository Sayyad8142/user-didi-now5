...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='bookings_select_self') THEN
    CREATE POLICY bookings_select_self ON public.bookings
      FOR SELECT USING (auth.uid() = user_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='bookings_insert_self') THEN
    CREATE POLICY bookings_insert_self ON public.bookings
      FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='bookings_select_admin') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='profiles_select_self') THEN
    CREATE POLICY profiles_select_self ON public.profiles
      FOR SELECT USING (auth.uid() = id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE polname='profiles_select_admin') THEN
