...
-- 1) Add updated_at column to ops_settings if missing
do $$
begin
  if not exists (
...
declare
  ...
  v_text  text;
begin
  -- Skip if no URL configured or placeholder
...
-- 4) Trigger function for support messages
...
declare
  ...
  v_preview text;
begin
  -- Only notify for user messages (sender = 'user')
  ...
  -- Get user context from thread -> profile
  ...
  -- Get booking context if thread is linked to a booking
  begin
